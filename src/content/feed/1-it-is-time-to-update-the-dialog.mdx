...
can’t we just call `hide: () => dialogRef.current.close(), show: () => dialogRef.current.showModal()`?

You could, for example reversing the control:
...
Technically yes, but also not. There’s a reason why [[RadixUI](https://www.radix-ui.com/docs/primitives/components/dialog)](https://www.radix-ui.com/docs/primitives/components/dialog) and [[React Spectrum](https://react-spectrum.adobe.com/react-spectrum/Dialog.html)](https://react-spectrum.adobe.com/react-spectrum/Dialog.html) wrap the `Dialog` into `ContextProvider` and provide trigger `Button` primitive. This allows connecting a trigger button to its dialog for screen readers. All of them also provide a way for to specify the description of the dialog for screen readers, like so:
...
As both elements can read from parent’s context, `<dialog>` can be connected to the `Heading` text via `aria-labelledby` and `id`:
...
Which you can also achieve with simpler code:
