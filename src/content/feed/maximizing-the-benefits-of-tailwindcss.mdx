...
poster: ./maximizing-the-benefits-of-tailwindcss/poster.jpeg
...
![a screenshot of a Guillermo Rauch tweet saying: "Tailwind thoroughly fixes CSS, by removing: Naming paralysis, Cascade side effects, Module import indirection, Declaration order side effects, Ergonomics](./maximizing-the-benefits-of-tailwindcss/rauch-tweet.png)
...
This is a simple example, but there's no way it's more maintainable than just CSS:
...
It's more code, but that's the point. I believe that a general rule for Front End development should be: **be more explicit in your source if it does not affect the resulting build**. And unless you work with monorepo or ship your component library pre-transpiled, you won't gain much from using plain Tailwind classes.
...
![A tweet from Adam Wathan that `@apply` brings many problems to the team, while `theme` syntax seem so much better](./maximizing-the-benefits-of-tailwindcss/adam-tweet.png)
